...
function initOrcamento() {
  ...
  function recalcularResumo() {
    ...
    // Desconto gerência (percentual adicional sobre o total já ajustado)
    ...
    if (descontoGerenciaCheckbox?.checked) {
      ...
      if (descontoGerenciaPercent > 100) descontoGerenciaPercent = 100;
      if (descontoGerenciaPercent > 0) {
        totalBase = totalBase * (1 - descontoGerenciaPercent / 100);
      }
    }
    ...
    if (ajusteLabel) {
      if (temJurosBase && (temDescontoBase || temDescontoEntrada || temDescontoGerencia)) {
        ajusteLabel.textContent = "Juros e descontos";
      } else if (temJurosBase) {
        ajusteLabel.textContent = "Juros";
      } else if (temDescontoBase || temDescontoEntrada || temDescontoGerencia) {
  ...
  if (entradaDescontoCheckbox) {
    entradaDescontoCheckbox.addEventListener("change", recalcularResumo);
  }
  ...
  btnGerarPdf.addEventListener("click", async () => {
    ...
    try {
      ...
      try {
        ...
        if (logoDataUrl) {
          doc.addImage(logoDataUrl, "PNG", 10, 6, 26, 20);
        }
      } catch (e) {
      ...
      doc.text("Moto Chefe", 40, 14);

      doc.setFontSize(11);
      doc.setTextColor(220);
      doc.text("Orçamento de veículos de mobilidade elétrica", 40, 22);
      ...
      // Resumo do orçamento (bem enxuto)
      doc.setFontSize(11);
      doc.text("Resumo do orçamento", 10, y);
      y += 6;

      // Descrição da forma de pagamento conforme calculadora
      // Regra: a partir de 13x não especificar se é com juros ou não, apenas a quantidade de parcelas
      let formaTextoPdf = "";
      ...
      // Forma de pagamento (discreto)
      doc.setFontSize(9.5);
      doc.text(`Forma de pagamento: ${formaTextoPdf}`, 10, y);
      y += 6;

      // Destaque principal: TOTAL
      doc.setFontSize(11);
      doc.setTextColor(20);
      doc.text("Total do orçamento", 10, y);
      doc.setFontSize(12.5);
      doc.text(formatCurrency(totalBase), 80, y);
      y += 7;

      // Se for parcelado, destacar valor de parcela
      if (dados.tipoPagamento === "parcelado") {
        doc.setFontSize(11);
        doc.text(
          `Parcelado em ${dados.numeroParcelas}x`,
          10,
          y
        );
        doc.setFontSize(12.5);
        doc.text(
          formatCurrency(valorParcelaFinal) + " / parcela",
          80,
          y
        );
        y += 7;

        // Informações adicionais de entrada / financiado (se existirem)
        doc.setFontSize(9.5);
        if (entradaPdfCheckbox?.checked && entradaValor > 0 && totalBase > 0) {
          const percEntradaPdf = (entradaValor / totalBase) * 100;
          const percEntradaTexto = percEntradaPdf.toFixed(1).replace(".", ",");
          doc.text(
            `Entrada (${percEntradaTexto}%): ${formatCurrency(entradaValor)}`,
            10,
            y
          );
          y += 4.5;
        ...
        if (financiadoValor > 0) {
          doc.text(
            `Valor financiado: ${formatCurrency(financiadoValor)}`,
            10,
            y
          );
          y += 5;
      ...
      // Frete (opcional, apenas se preenchido)
      if (freteValor > 0) {
        doc.setFontSize(9.5);
        doc.text(`Frete: ${formatCurrency(freteValor)}`, 10, y);
        y += 5;
      }
      ...
      dados.produtos.forEach((p) => {
        ...
        doc.text(formatCurrency(p.subtotal), 145, y);
        y += 5;
      });
      ...
      doc.save(`orcamento-${nome.replace(/\s+/g, "-").toLowerCase()}.pdf`);
    } catch (err) {
