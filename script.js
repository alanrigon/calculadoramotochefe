...
function initOrcamento() {
  ...
  btnGerarPdf.addEventListener("click", async () => {
    ...
    try {
      ...
      // Calcular total de descontos (somar todos)
      let totalDescontos = 0;
      
      // Desconto base da calculadora (se houver)
      if (dados.ajuste < 0) {
        totalDescontos += Math.abs(dados.ajuste);
      }

      // Desconto de 2,5% por entrada (se checkbox marcado)
      if (elegivelDesconto && entradaDescontoCheckbox?.checked) {
      ...
      // Adicionar linha única de Desconto se houver algum desconto
      if (totalDescontos > 0) {
        linhasTabela.push({
          descricao: "Desconto",
