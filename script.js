...
function initOrcamento() {
  ...
  btnGerarPdf.addEventListener("click", async () => {
    ...
    try {
      ...
      // Cabeçalho da tabela (fundo preto, texto branco)
      const yHeader = y;
      ...
      // Juros/Desconto base da calculadora
      if (dados.ajuste !== 0) {
        linhasTabela.push({
          descricao: dados.ajuste > 0 ? "Juros" : "Desconto",
          valor: dados.ajuste,
          negrito: false
        });
      }

      // Desconto de 2,5% por entrada (se aplicado)
      if (elegivelDesconto && entradaDescontoCheckbox?.checked) {
        const descontoEntrada = totalOriginal - (totalOriginal * 0.975);
        linhasTabela.push({
          descricao: "Desconto entrada (2,5%)",
          valor: -descontoEntrada,
          negrito: false
        });
      }

      // Desconto gerência (se aplicado)
      ...
      if (dados.tipoPagamento === "parcelado") {
        ...
        if (entradaPdfCheckbox?.checked && entradaValor > 0 && totalBase > 0) {
          ...
          linhasTabela.push({
            descricao: `Entrada (${percEntradaTexto}%)`,
            valor: entradaValor,
            negrito: false
          });
        }
      ...
      // Total em caixa preta destacada
      const yTotal = y;
      const alturaTotal = 8;
      doc.setFillColor(0, 0, 0);
      // Caixa preta para o total (largura completa da tabela)
      doc.rect(colDescricao, yTotal, larguraTabela - colDescricao, alturaTotal, "F");
      doc.setFontSize(11);
      doc.setTextColor(255);
      doc.setFont(undefined, "bold");
      doc.text("Total", colDescricao + 2, yTotal + 5);
      // Valor do total alinhado à direita
